...
# tensorflow_addons removed (deprecated/incompatible) - use tf.keras alternatives
...
class Critic(tf.Module):
  def __init__(self, num_features, hidden_size, activation=tf.nn.relu, dropout_prob=0.1):
    ...
    self.activation = activation
    self.dropout_prob = dropout_prob
    # Use built-in Keras GroupNormalization (no tensorflow_addons needed)
    self.group_norm = tf.keras.layers.GroupNormalization(groups=1)
    
  def __call__(self, state):
    #[I, P] --> [I]
    layer_output = self.layer1(state)
    layer_output = self.group_norm(layer_output)
    layer_output = self.activation(layer_output)
    layer_output = tf.nn.dropout(layer_output, self.dropout_prob)
...
# import tensorflow as tf
# # tensorflow_addons không còn cần thiết
...
#     self.activation = activation
#     self.dropout_prob = dropout_prob
#     # Thay thế tfa.layers.GroupNormalization
#     self.group_norm = tf.keras.layers.GroupNormalization(groups=1)
    
#   def __call__(self, state):
#     layer_output = self.layer1(state)
#     # Dùng GroupNormalization từ Keras
#     layer_output = self.group_norm(layer_output)
#     layer_output = self.activation(layer_output)
#     layer_output = tf.nn.dropout(layer_output, self.dropout_prob)
#     layer_output = self.layer2(layer_output)
...
def quantile(x, q):
  """Compute quantile using TensorFlow (linear interpolation)"""
  x_sorted = tf.sort(tf.reshape(x, [-1]), direction='ASCENDING')
  n = tf.cast(tf.shape(x_sorted)[0], tf.float32)
  
  # Handle edge case: empty or single element
  n_int = tf.shape(x_sorted)[0]
  if_empty = tf.constant(0.0, dtype=tf.float32)
  ...
  def compute_quantile():
    ...
    # Ensure int_pos doesn't go out of bounds
    int_pos = tf.minimum(int_pos, n_int - 2)
    int_pos = tf.maximum(int_pos, 0)
    
    v1 = tf.gather(x_sorted, int_pos)
    v2 = tf.gather(x_sorted, int_pos + 1)
    p_diff = pos - floor_pos
    
    return v1 + p_diff * (v2 - v1)
  
  return tf.cond(n_int > 0, compute_quantile, lambda: if_empty)

def cross_entropy(p, q):
  return -tf.reduce_mean(tf.reduce_sum(p*tf.math.log(tf.math.maximum(1e-15, q)), axis=1))

# Class Env removed - unused and contained errors (wrong super() call, undefined 'state', tfa reference)
...
def predict():
  ...
  with tf.io.gfile.GFile(FLAGS.output_file, "w") as writer:
    for sales_record in parsed_dataset:
      ...
      stockout = tf.math.maximum(0, sales - x_u)
